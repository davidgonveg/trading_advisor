...
class SignalScanner:
    ...
    def __init__(self, data_manager=None, time_provider=None):
        """Inicializar el scanner con todos los componentes"""
        self.indicators = TechnicalIndicators()
        
        # Inyección de dependencias (o defaults para modo Live)
    ...
    def is_market_open(self) -> bool:
        ...
        """
        try:
            now = self.time_provider.now(self.market_tz)
            ...
            enable_extended = config.CONTINUOUS_DATA_CONFIG.get('ENABLE_EXTENDED_HOURS', False)
            
            sessions_to_check = config.EXTENDED_TRADING_SESSIONS if enable_extended else config.TRADING_SESSIONS
            
            is_open = False
            for session_name, session_config in sessions_to_check.items():
                # Skip if session explicitly disabled (only in extended dict)
                if enable_extended and not session_config.get('ENABLED', False):
                    continue
                    
                # Get start/end (handle both config formats)
                start_str = session_config.get('START')
                end_str = session_config.get('END')
                ...
                try:
                    # Check interval
                    if session_name == 'OVERNIGHT' or start_time > end_time:
                         # Crosses midnight (e.g. 20:00 to 04:00)
                         if current_time >= start_time or current_time <= end_time:
                             is_open = True
                             break
                    else:
                         # Normal day (e.g. 09:30 to 16:00)
                         if current_time >= start_time and current_time <= end_time:
                             is_open = True
                             break
                except Exception as e:
                    logger.error(f"❌ Error comparison in is_market_open: {e}")
                    logger.error(f"   Types: current_time={type(current_time)} start_time={type(start_time)}")
                    raise e
    ...
    def scan_symbol(self, symbol: str) -> Optional[TradingSignal]:
        ...
        try:
            ...
            timestamp = self.time_provider.now(self.market_tz)
    ...
    def scan_multiple_symbols(self, symbols: List[str] = None) -> List[TradingSignal]:
        ...
        try:
            ...
            self.scan_count += 1
            self.last_scan_time = self.time_provider.now(self.market_tz)
